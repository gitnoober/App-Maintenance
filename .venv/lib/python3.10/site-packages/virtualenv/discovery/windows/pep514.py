...
def load_exe(hive_name, company, company_key, tag):
    ...
    try:
        with winreg.OpenKeyEx(
            company_key, r"{}\InstallPath".format(tag)
        ) as ip_key:
            with ip_key:
                ...
                if exe is not None and os.path.exists(exe):
                    ...
                    msg(
                        key_path,
                        "could not load exe with value {}".format(exe),
                    )
    except OSError:
...
def load_version_data(hive_name, company, tag, tag_key):
    for candidate, key_path in [
        (
            ...
            "{}/{}/{}/SysVersion".format(hive_name, company, tag),
        ),
...
def msg(path, what):
    LOGGER.warning(
        "PEP-514 violation in Windows Registry at {} error: {}".format(
            path, what
        )
    )
