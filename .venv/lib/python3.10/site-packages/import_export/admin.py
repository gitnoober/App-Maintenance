...
class ImportMixin(BaseImportMixin, ImportExportMixinBase):
    ...
    def process_dataset(self, dataset, confirm_form, request, *args, **kwargs):
        ...
        res_kwargs = self.get_import_resource_kwargs(
            request, form=confirm_form, *args, **kwargs
        )
    ...
    def import_action(self, request, *args, **kwargs):
        ...
        if request.POST and form.is_valid():
            ...
            try:
                ...
                return HttpResponse(
                    _("<h1>Imported file has a wrong encoding: %s</h1>" % e)
                )
            except Exception as e:
                return HttpResponse(
                    _(
                        "<h1>%s encountered while trying to read file: %s</h1>"
    ...
    def changelist_view(self, request, extra_context=None):
        if extra_context is None:
            extra_context = {}
        extra_context["has_import_permission"] = self.has_import_permission(
            request
        )
...
class ExportMixin(BaseExportMixin, ImportExportMixinBase):
    ...
    def changelist_view(self, request, extra_context=None):
        if extra_context is None:
            extra_context = {}
        extra_context["has_export_permission"] = self.has_export_permission(
            request
        )
        return super().changelist_view(request, extra_context)
