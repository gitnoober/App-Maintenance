...
class SharedStringTable(object):
    ...
    def get_biff_record(self):
        ...
        del data
        self._new_piece()
        self._continues[0] = pack(
            "<2HII",
            self._SST_ID,
            len(self._sst_record),
            self._add_calls,
            len(self._str_indexes) + len(self._rt_indexes),
        )
    ...
    def get(self):
        ...
        if len(data) > 0x2020:  # limit for BIFF7/8
            ...
            for chunk in chunks[1:]:
                continues += pack(
                    "<2H%ds" % len(chunk), 0x003C, len(chunk), chunk
                )
...
class WriteAccessRecord(BiffRecord):
    ...
    def __init__(self, owner):
        ...
        if isinstance(uowner, unicode_type):
            uowner = uowner.encode(
                "ascii"
            )  # probably not ascii, but play it safe until we know more
        self._rec_data = pack(
            "%ds%ds" % (uowner_len, 0x70 - uowner_len),
            uowner,
            b" " * (0x70 - uowner_len),
        )
...
class XFRecord(BiffRecord):
    ...
    def __init__(self, xf, xftype="cell"):
        ...
        self._rec_data = (
            fnt + fmt + prt + aln + rot + txt + used_attr + brd1 + brd2 + pat
        )
...
class ExtSSTRecord(BiffRecord):
    ...
    def __init__(self, sst_stream_pos, str_placement, portions_len):
        ...
        exsst_str_count_delta = max(
            8, len(str_placement) * 8 / 0x2000
        )  # maybe smth else?
        self._rec_data = pack("<H", exsst_str_count_delta)
        str_counter = 0
        while str_counter < len(str_placement):
            self._rec_data += pack(
                "<IHH", extsst[str_counter][1], extsst[str_counter][0], 0
            )
...
class DimensionsRecord(BiffRecord):
    ...
    def __init__(
        self, first_used_row, last_used_row, first_used_col, last_used_col
    ):
...
class PanesRecord(BiffRecord):
    ...
    def __init__(self, px, py, first_row_bottom, first_col_right, active_pane):
        allowed = self.valid_active_pane.get((int(px > 0), int(py > 0)))
        if active_pane not in allowed:
            raise ValueError(
                "Cannot set active_pane to %i, must be one of %s"
                % (active_pane, ", ".join(allowed))
            )
        self._rec_data = pack(
            "<5H", px, py, first_row_bottom, first_col_right, active_pane
        )
    ...
    def __init__(self, merged_list):
        ...
        while i >= 0:
            ...
            while (i >= 0) and (j < 0x403):
                ...
                j += 1
            self._rec_data += (
                pack("<3H", self._REC_ID, len(merged) + 2, j) + merged
            )
...
class MulBlankRecord(BiffRecord):
    ...
    def __init__(self, row, first_col, last_col, xf_index):
        ...
        self._rec_data = pack(
            "<%dH" % blanks_count, *([xf_index] * blanks_count)
        )
        self._rec_data = (
            pack("<2H", row, first_col) + self._rec_data + pack("<H", last_col)
        )
...
class ColInfoRecord(BiffRecord):
    ...
    def __init__(self, first_col, last_col, width, xf_index, options, unused):
        self._rec_data = pack(
            "<6H", first_col, last_col, width, xf_index, options, unused
        )
...
class NameRecord(BiffRecord):
    ...
    def __init__(
        ...
        self._rec_data = (
            pack(
                "<HBBHHHBBBBB%ds%ds" % (uname_len, len(rpn)),
                options,
                keyboard_shortcut,
                uname_len,
                len(rpn),
