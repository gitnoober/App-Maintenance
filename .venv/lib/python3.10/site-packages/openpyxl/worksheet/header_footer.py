...
FORMAT_REGEX = re.compile(
    "{0}|{1}|{2}".format(FONT_PATTERN, COLOR_PATTERN, SIZE_REGEX)
)
...
class _HeaderFooterPart(Strict):
    ...
    * &nn   Selects the specified 2-digit font point size
    ...
    def from_str(cls, text):
        ...
        kw = dict(
            (k, v)
            for match in FORMAT_REGEX.findall(text)
...
class HeaderFooter(Serialisable):
    ...
    def __bool__(self):
        parts = [
            getattr(self, attr) for attr in self.__attrs__ + self.__elements__
        ]
        return any(parts)
