...
class StrCell(object):
    ...
    def get_biff_data(self):
        ...
        return pack(
            "<5HL", 0x00FD, 10, self.rowx, self.colx, self.xf_idx, self.sst_idx
        )
...
class MulBlankCell(object):
    ...
    def get_biff_data(self):
        return BIFFRecords.MulBlankRecord(
            self.rowx, self.colx1, self.colx2, self.xf_idx
        ).get()
...
class NumberCell(object):
    ...
    def get_encoded_data(self):
        ...
        # print
        return 0, pack(
            "<5Hd", 0x0203, 14, self.rowx, self.colx, self.xf_idx, num
        )
    ...
    def get_biff_data(self):
        ...
        if isRK:
            return pack(
                "<5Hi", 0x27E, 10, self.rowx, self.colx, self.xf_idx, value
            )
...
class BooleanCell(object):
    ...
    def get_biff_data(self):
        return BIFFRecords.BoolErrRecord(
            self.rowx, self.colx, self.xf_idx, self.number, 0
        ).get()
...
class ErrorCell(object):
    ...
    def get_biff_data(self):
        return BIFFRecords.BoolErrRecord(
            self.rowx, self.colx, self.xf_idx, self.number, 1
        ).get()
...
class FormulaCell(object):
    ...
    def get_biff_data(self):
        return BIFFRecords.FormulaRecord(
            self.rowx,
            self.colx,
            self.xf_idx,
            self.frmla.rpn(),
    ...
    while i < nitems:
        ...
        if target == NumberCell:
            if lastcolx == icolx:
                ...
                pieces.append(
                    pack("<5Hi", 0x027E, 10, rowx, icolx, xf_idx, value)
                )
            else:
                # MULRK record
                nc = lastcolx - icolx + 1
                ...
                pieces.append(
                    b"".join(
                        pack("<Hi", xf_idx, value) for value, xf_idx in muldata
                    )
                )
            ...
            if lastcolx == icolx:
                ...
                pieces.append(
                    b"".join(pack("<H", xf_idx) for xf_idx in muldata)
                )
                pieces.append(pack("<H", lastcolx))
        if packed_record:
