...
from .cell_range import CellRange, MultiCellRange
...
class Worksheet(_WorkbookChild):
    ...
    def freeze_panes(self, topLeftCell=None):
        ...
        view.pane = Pane(
            topLeftCell=topLeftCell, activePane="topRight", state="frozen"
        )
        ...
        if row > 1 and column > 1:
            ...
            sel.insert(
                0, Selection(pane="topRight", activeCell=None, sqref=None)
            )
            sel.insert(
    ...
    def _cells_by_row(
        self, min_col, min_row, max_col, max_row, values_only=False
    ):
    ...
    def add_table(self, table):
        ...
        if self.parent._duplicate_name(table.name):
            raise ValueError(
                "Table with name {0} already exists".format(table.name)
            )
        ...
        if isinstance(iterable, (list, tuple, range)) or isgenerator(iterable):
            for col_idx, content in enumerate(iterable, 1):
                if isinstance(content, Cell):
                    ...
                    cell = Cell(
                        self, row=row_idx, column=col_idx, value=content
                    )
    ...
    def _move_cells(
        self, min_row=None, min_col=None, offset=0, row_or_col="row"
    ):
        """
    ...
    def delete_rows(self, idx, amount=1):
        ...
        self._move_cells(
            min_row=idx + amount, offset=-amount, row_or_col="row"
        )
    ...
    def delete_cols(self, idx, amount=1):
        ...
        self._move_cells(
            min_col=idx + amount, offset=-amount, row_or_col="column"
        )
...
def _gutter(idx, offset, max_val):
    ...
    """
    gutter = range(
        max(max_val + 1 - offset, idx), min(idx + offset, max_val) + 1
    )
    return gutter
