...
def parse_args(check=True):
    ...
    if options.config_file is None:
        ...
        if not os.path.exists(options.config_file):
            parser.error(
                "Config file '{0}' doesn't exist!".format(options.config_file)
            )
...
def download_node_src(node_url, src_dir, opt):
    ...
    try:
        ...
        logger.warning(
            "Incomplete read while reading" "from {}".format(node_url)
        )
...
def install_node_wrapped(env_dir, src_dir, opt):
    ...
    logger.info(
        " * Install %s node (%s) " % (src_type, opt.node),
...
def install_npm(env_dir, _src_dir, opt):
    ...
    """
    logger.info(
        " * Install npm.js (%s) ... " % opt.npm, extra=dict(continued=True)
    )
    env = dict(
        os.environ,
...
def install_packages(env_dir, opt):
    ...
    packages = [
        package.strip() for package in open(opt.requirements).readlines()
    ]
