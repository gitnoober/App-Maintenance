...
def _range_seq_to_re(range_seq):
    ...
    return re.compile(
        "[{}]".format(
            "".join(
                "{}-{}".format(re.escape(unichr(lo)), re.escape(unichr(hi)))
...
_xml_filtered_chars_re = _range_seq_to_re(
    _xml10_illegal_ranges + _xml_discouraged_ranges
)
...
class CDATASection(Text, Childless):
    ...
    def toXml(self, level, f):
        ...
        if self.data:
            f.write(
                "<![CDATA[%s]]>" % self.data.replace("]]>", "]]>]]><![CDATA[")
            )
...
class Element(Node):
    ...
    def __init__(
        ...
        **args
        ...
        if allowed_attrs is not None:
            allowed_args = [
                a[1].lower().replace("-", "") for a in allowed_attrs
            ]
        self.attributes = {}
    ...
    def addText(self, text, check_grammar=True):
        ...
        if check_grammar and self.qname not in grammar.allows_text:
            raise IllegalText(
                "The <%s> element does not allow text" % self.tagName
            )
        else:
            if text != "":
    ...
    def write_open_tag(self, level, f):
        ...
        if level == 0:
            for namespace, prefix in self.namespaces.items():
                f.write(
                    " xmlns:" + prefix + '="' + _sanitize(str(namespace)) + '"'
                )
        for qname in self.attributes.keys():
            prefix = self.get_nsprefix(qname[0])
            f.write(
                " "
                + _sanitize(str(prefix + ":" + qname[1]))
                + "="
                + _quoteattr(unicode(self.attributes[qname]))
            )
        f.write(">")
    ...
    def toXml(self, level, f):
        ...
        for qname in self.attributes.keys():
            ...
            f.write(
                " "
                + _sanitize(unicode(prefix + ":" + qname[1]))
                + "="
                + _quoteattr(unicode(self.attributes[qname]))
            )
        if self.childNodes:
