...
from openpyxl.packaging.workbook import ChildSheet, PivotCache, WorkbookPackage
...
def get_active_sheet(wb):
    ...
    visible_sheets = [
        idx
        for idx, sheet in enumerate(wb._sheets)
    ...
    def write_names(self):
        ...
        for idx, sheet in enumerate(self.wb.worksheets):
            ...
            if sheet.print_titles:
                ...
                name.value = ",".join(
                    [
                        "{0}!{1}".format(quote_sheetname(sheet.title), r)
    ...
    def write_pivots(self):
        ...
        for pivot in self.wb._pivots:
            if pivot.cache not in pivot_caches:
                ...
                rel = Relationship(
                    Type=pivot.cache.rel_type, Target=pivot.cache.path
                )
                self.rels.append(rel)
                c.id = rel.id
