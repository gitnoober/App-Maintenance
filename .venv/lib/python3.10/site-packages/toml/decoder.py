...
def loads(s, _dict=dict, decoder=None):
    ...
    if keyname:
        raise TomlDecodeError(
            "Key name found without value." " Reached end of file.",
            original,
            len(s),
        )
    if openstring:  # reached EOF and have an unterminated string
        raise TomlDecodeError(
            "Unterminated string found." " Reached end of file.",
            original,
            len(s),
        )
    s = "".join(sl)
        ...
        if line[0] == "[":
            ...
            while i < len(groups):
                ...
                if len(groups[i]) > 0 and (
                    ...
                    groupstr = groups[i]
                    j = i + 1
                    while (not groupstr[0] == groupstr[-1]) or len(
                        groupstr
                    ) == 1:
                        j += 1
                        if j > len(groups) + 2:
...
class TomlDecoder(object):
    ...
    def _load_line_multiline_str(self, p):
        ...
        if len(p) < 3:
            return -1, poffset
        if p[0] == "[" and (
            p.strip()[-1] != "]" and self._load_array_isstrarray(p)
        ):
