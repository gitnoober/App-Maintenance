...
def is_date_format_string(book, fmt):
    ...
    if book.verbosity >= 4:
        print(
            "is_date_format_string: reduced format is %s" % REPR(s),
            file=book.logfile,
        )
...
def handle_palette(book, data):
    ...
    if not expected_size <= actual_size <= expected_size + tolerance:
        raise XLRDError(
            "PALETTE record: expected size %d, actual size %d"
    ...
    for i in xrange(n_colours):
        ...
        if blah:
            if new_rgb != old_rgb:
                print(
                    "%2d: %r -> %r" % (i, old_rgb, new_rgb), file=book.logfile
                )
...
def handle_xf(self, data):
    ...
    if bv >= 80:
        ...
        upkbitsL(
            xf.background, pkd_brdbkg2, ((26, 0xFC000000, "fill_pattern"),)
        )
        upkbits(
            xf.background,
            pkd_brdbkg3,
    ...
    if xf.format_key not in self.format_map:
        ...
        if self.verbosity:
            fprintf(
                self.logfile, msg, xf.xf_index, xf.format_key, xf.format_key
            )
        xf.format_key = 0
