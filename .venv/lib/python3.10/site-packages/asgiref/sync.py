...
class AsyncToSync:
    ...
    def __init__(self, awaitable, force_new_loop=False):
        if not callable(awaitable) or not _iscoroutinefunction_or_partial(
            awaitable
        ):
...
class SyncToAsync:
    ...
    if sys.version_info >= (3, 7):
        deadlock_context: "contextvars.ContextVar[bool]" = (
            contextvars.ContextVar("deadlock_context")
        )
    else:
    ...
    async def __call__(self, *args, **kwargs):
        ...
        if self._thread_sensitive:
            if hasattr(AsyncToSync.executors, "current"):
                ...
                self.thread_sensitive_context
                and self.thread_sensitive_context.get(None)
            ):
