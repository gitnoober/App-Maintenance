...
class Workbook(object):
    ...
    def set_colour_RGB(self, colour_index, red, green, blue):
        ...
        if min(red, green, blue) < 0 or max(red, green, blue) > 255:
            raise Exception(
                "set_colour_RGB: colour values (%d,%d,%d) must be in range(0, 256)"
                % (red, green, blue)
            )
    ...
    def convert_sheetindex(self, strg_ref, n_sheets):
        ...
        if 0 <= idx < n_sheets:
            return idx
        msg = "Formula: sheet index (%s) >= number of sheets (%d)" % (
            strg_ref,
            n_sheets,
        )
        raise Exception(msg)
    ...
    def setup_ownbook(self):
        ...
        self.__sheet_refs[reference] = self._ownbook_supbook_ref = len(
            self.__sheet_refs
        )
    ...
    def setup_xcall(self):
        ...
        if reference in self.__sheet_refs:
            raise Exception("can't happen")
        self.__sheet_refs[reference] = self._xcall_supbook_ref = len(
            self.__sheet_refs
        )
    ...
    def __bof_rec(self):
        return BIFFRecords.Biff8BOFRecord(
            BIFFRecords.Biff8BOFRecord.BOOK_GLOBAL
        ).get()
    ...
    def __window1_rec(self):
        ...
        return BIFFRecords.Window1Record(
            ...
            flags,
            self.__active_sheet,
            self.__first_tab_index,
            self.__selected_tabs,
            self.__tab_width_twips,
        ).get()
    ...
    def __country_rec(self):
        if not self.__country_code:
            return b""
        return BIFFRecords.CountryRecord(
            self.__country_code, self.__country_code
        ).get()
    ...
    def __boundsheets_rec(
        self, data_len_before, data_len_after, sheet_biff_lens
    ):
        #  .................................
        # BOUNDSEHEET0
        # BOUNDSEHEET1
            ...
            if stype == "ownbook":
                ...
                for idx, name in temp:
                    rec = BIFFRecords.ExternnameRecord(
                        options=0, index=0, name=name, fmla="\x02\x00\x1c\x17"
                    ).get()
                    pieces.append(rec)
            else:
    ...
    def get_biff_data(self):
        ...
        ext_sst = self.__ext_sst_rec(
            0
        )  # need fake cause we need calc stream pos
        eof = self.__eof_rec()
        ...
        bundlesheets = self.__boundsheets_rec(
            len(before), len(after) + len(ext_sst) + len(eof), sheet_biff_lens
        )

        sst_stream_pos = (
            len(before) + len(bundlesheets) + len(country) + len(all_links)
        )
