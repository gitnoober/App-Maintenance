...
class Name(BaseObject):
    ...
    def area2d(self, clipped=True):
        ...
        raise XLRDError(
            "Not a constant absolute reference to a single area in a single sheet"
        )
...
class Book(BaseObject):
    ...
    def biff2_8_load(
        ...
        if not file_contents:
            with open(filename, "rb") as f:
                ...
                if self.use_mmap:
                    self.filestr = mmap.mmap(
                        f.fileno(), size, access=mmap.ACCESS_READ
                    )
        ...
        if DEBUG:
            print(
                "mem: %s, base: %d, len: %d"
                % (type(self.mem), self.base, self.stream_len),
                file=self.logfile,
            )
    ...
    def fake_globals_get_sheet(self):  # for BIFF 4.0 and earlier
        ...
        self._sheet_list.append(
            None
        )  # get_sheet updates _sheet_list but needs a None beforehand
        self.get_sheets()
    ...
    def handle_boundsheet(self, data):
        ...
        if bv == 45:  # BIFF4W
            ...
            abs_posn = (
                offset + self.base
            )  # because global BOF is always at posn 0 in the stream
    ...
    def derive_encoding(self):
        if self.encoding_override:
            ...
            if DEBUG or (self.verbosity and encoding != self.encoding):
                fprintf(
                    self.logfile,
                    "CODEPAGE: codepage %r -> encoding %r\n",
                    codepage,
                    encoding,
                )
    ...
    def handle_externsheet(self, data):
        ...
        if self.biff_version >= 80:
            ...
            if blah2:
                ...
                print(
                    "   %3d chars, type is %d (%s)" % (nc, ty, msg),
                    file=self.logfile,
                )
            if ty == 3:
    ...
    def handle_name(self, data):
        ...
        if bv < 80:
            ...
            internal_name, pos = unpack_unicode_update_pos(
                data, 14, known_len=name_len
            )
    ...
    def names_epilogue(self):
        ...
        if blah:
            ...
            print(
                ...
                REPR(self._extnsht_name_from_num),
                file=f,
            )
            print(
                "_sheet_num_from_name", REPR(self._sheet_num_from_name), file=f
            )
            ...
            if self.biff_version >= 80:
                ...
                if sheet_index == 0:
                    ...
                    if intl_sheet_index == -1:  # maps to a macro or VBA sheet
                        intl_sheet_index = (
                            -2
                        )  # valid sheet reference but not useful
                else:
                    # huh?
                ...
                if sheet_index == 0:
                    ...
                    intl_sheet_index = self._sheet_num_from_name.get(
                        sheet_name, -2
                    )
        ...
        name_map = (
            {}
        )  # name.lower() : list of Name_objects (sorted in scope order)
        for namex in range(num_names):
            nobj = self.name_obj_list[namex]
    ...
    def handle_supbook(self, data):
        ...
        if data[2:4] == b"\x01\x04":
            ...
            if blah:
                print(
                    "SUPBOOK[%d]: internal 3D refs; %d sheets"
                    % (sbn, num_sheets),
                    file=self.logfile,
                )
                print(
    ...
    def handle_sheethdr(self, data):
        ...
        if DEBUG:
            print(
                "SHEETHDR: xf epilogue flag is %d" % self._xf_epilogue_done,
                file=self.logfile,
            )
        self._sheet_list.append(
            None
    ...
    def handle_sst(self, data):
        ...
        while 1:
            ...
            if DEBUG >= 2:
                fprintf(
                    self.logfile,
                    "CONTINUE: adding %d bytes to SST -> %d\n",
                    nb,
                    nbt,
                )
            strlist.append(data)
        self._sharedstrings, rt_runlist = unpack_SST_table(
            strlist, uniquestrings
        )
        ...
        if DEBUG:
            t1 = perf_counter()
            print(
                "SST processing took %.2f seconds" % (t1 - t0,),
                file=self.logfile,
            )
    ...
    def parse_globals(self):
        ...
        while 1:
            ...
            if DEBUG:
                print(
                    "parse_globals: record code is 0x%04x" % rc,
                    file=self.logfile,
                )
            if rc == XL_SST:
                ...
                rc == XL_FORMAT
            ):  # XL_FORMAT2 is BIFF <= 3.0, can't appear in globals
    ...
    def getbof(self, rqd_stream):
        ...
        if opcode not in bofcodes:
            bof_error(
                "Expected BOF record; found %r" % self.mem[savpos : savpos + 8]
            )
        ...
        if DEBUG:
            print(
                "getbof(): BOF found at offset %d; savpos=%d"
                % (bof_offset, savpos),
                file=self.logfile,
            )
        ...
        if (
            rqd_stream == XL_WORKBOOK_GLOBALS and got_globals
        ) or streamtype == rqd_stream:
            return version
