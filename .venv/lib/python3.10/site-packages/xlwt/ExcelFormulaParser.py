...
class Parser(antlr.LLkParser):
    ...
    def primary(self, arg_type):
        ...
        if False:
            ...
            choose_rpn = []
            choose_rpn.append(
                struct.pack("<BBH", ptgAttr, 0x04, nc)
            )  # 0x04 is tAttrChoose
            choose_rpn.append(struct.pack("<%dH" % (nc + 1), *jump_pos))
            for ic in xrange(nc):
                choose_rpn.append(rpn_chunks[ic])
                choose_rpn.append(
                    struct.pack("<BBH", ptgAttr, 0x08, skiplens[ic])
                )  # 0x08 is tAttrSkip
            choose_rpn.append(
                struct.pack("<BBH", ptgFuncVarV, nc + 1, 100)
            )  # 100 is CHOOSE fn
            ...
            if (self.LA(1) == INT_CONST) and (_tokenSet_0.member(self.LA(2))):
                ...
                self.LA(1) == INT_CONST
                or self.LA(1) == NAME
                or self.LA(1) == QUOTENAME
            ) and (self.LA(2) == COLON or self.LA(2) == BANG):
                pass
                sheet1 = self.sheet()
                sheet2 = sheet1
                la1 = self.LA(1)
                ...
                self.match(NAME)
                raise Exception(
                    "[formula] found unexpected NAME token (%r)" % name_tok.txt
                )
                ...
                if func_toku in all_funcs_by_name:
                    (
                        ...
                        func_type,
                        arg_type_str,
                    ) = all_funcs_by_name[func_toku]
                    arg_type_list = list(arg_type_str)
                else:
                    raise Exception(
                        "[formula] unknown function (%s)" % func_tok.text
                    )
                ...
                if arg_count > max_argc or arg_count < min_argc:
                    raise Exception(
                        "%d parameters for function: %s"
                        % (arg_count, func_tok.text)
                    )
                if xcall:
                    func_ptg = ptgFuncVarR + _RVAdelta[func_type]
                    self.rpn += struct.pack(
                        "<2BH", func_ptg, arg_count + 1, 255
                    )  # 255 is magic XCALL function
