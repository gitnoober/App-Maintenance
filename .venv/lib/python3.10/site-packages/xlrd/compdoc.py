...
class CompDoc(object):
    ...
    def __init__(
        ...
        if mem[28:30] != b"\xFE\xFF":
            raise CompDocError(
                'Expected "little-endian" marker, found %r' % mem[28:30]
            )
        ...
        if self.sec_size != 512 or self.short_sec_size != 64:
            print(
                "@@@@ sec_size=%d short_sec_size=%d"
                % (self.sec_size, self.short_sec_size),
                file=logfile,
            )
        (
            SAT_tot_secs,
        ...
        expected_MSATX_sectors = max(
            0, (SAT_sectors_reqd - 109 + nent - 2) // (nent - 1)
        )
        ...
        if MSATX_tot_secs == 0 and MSATX_first_sec_sid in (EOCSID, FREESID, 0):
            ...
            while sid not in (EOCSID, FREESID, MSATSID):
                ...
                if sid >= mem_data_secs:
                    ...
                    raise CompDocError(
                        "MSAT extension: invalid sector id: %d" % sid
                    )
                if seen[sid]:
                    raise CompDocError(
                        "MSAT corruption: seen[%d] == %d" % (sid, seen[sid])
                    )
                seen[sid] = 1
                ...
                sid = (
                    MSAT.pop()
                )  # last sector id is sid of next sector in the chain
        ...
        if sscs_dir.tot_size > 0:
            ...
            while sid >= 0 and nsecs > 0:
                if seen[sid]:
                    raise CompDocError(
                        "SSAT corruption: seen[%d] == %d" % (sid, seen[sid])
                    )
                seen[sid] = 5
                nsecs -= 1
            ...
            if DEBUG:
                print(
                    "SSAT last sid %d; remaining sectors %d" % (sid, nsecs),
                    file=logfile,
                )
        ...
        for child in dl[storage_DID].children:
            if dl[child].name.lower() == head.lower():
                ...
                if et == 1:
                    if not tail:
                        raise CompDocError(
                            "Requested component is a 'storage'"
                        )
    ...
    def locate_named_stream(self, qname):
        ...
        if d.tot_size > self.mem_data_len:
            raise CompDocError(
                "%r stream length (%d bytes) > file data size (%d bytes)"
    ...
    def _locate_stream(
        ...
        while s >= 0:
            if self.seen[s]:
                if not self.ignore_workbook_corruption:
                    print(
                        "_locate_stream(%s): seen" % qname, file=self.logfile
                    )
                    dump_list(self.seen, 20, self.logfile)
                    raise CompDocError(
                        "%s corruption: seen[%d] == %d"
        ...
        return (
            b"".join(mem[start_pos:end_pos] for start_pos, end_pos in slices),
            0,
            expected_stream_size,
        )
