...
class DbfRecord:
    ...
    def rawFromStream(cls, dbf, index):
        ...
        dbf.stream.seek(
            dbf.header.headerLength + index * dbf.header.recordLength
        )
    ...
    def __repr__(self):
        ...
        for _fld in self.dbf.fieldNames:
            ...
            if _val is utils.INVALID_VALUE:
                _rv.append(
                    _template % (_fld, "None", "value cannot be decoded")
                )
            else:
    ...
    def _write(self):
        ...
        self.dbf.stream.write(
            bytes(self.toString(), sys.getfilesystemencoding())
        )
    ...
    def _validateIndex(self, allowUndefined=True, checkRange=False):
        ...
        if self.index is None:
            ...
            raise ValueError(
                "There are only %d records in the DBF"
