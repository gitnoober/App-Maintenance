...
class StyleToCSS:
    ...
    def __init__(self):
        ...
        self.ruleconversions = {
            ...
            ): self.c_text_line_through_style,
    ...
    def c_fn(self, ruleset, sdict, rule, fontstyle):
        ...
        family, htmlgeneric = self.fontdict.get(
...
class ODF2XHTML(handler.ContentHandler):
    ...
    def __init__(self, generate_css=True, embedable=False):
        ...
        self.elements = {
            ...
            (OFFICENS, "automatic-styles"): (
                self.s_office_automatic_styles,
                None,
            ),
            (OFFICENS, "document"): (
                self.s_office_document_content,
                self.e_office_document_content,
            ),
            ...
            (OFFICENS, "presentation"): (
                self.s_office_presentation,
                self.e_office_presentation,
            ),
            (OFFICENS, "spreadsheet"): (
                self.s_office_spreadsheet,
                self.e_office_spreadsheet,
            ),
            ...
            (STYLENS, "default-style"): (
                self.s_style_default_style,
                self.e_style_default_style,
            ),
            (STYLENS, "drawing-page-properties"): (
                self.s_style_handle_properties,
                None,
            ),
            ...
            (TEXTNS, "list-item"): (
                self.s_text_list_item,
                self.e_text_list_item,
            ),
            ...
            (TEXTNS, "note-body"): (
                self.s_text_note_body,
                self.e_text_note_body,
            ),
    ...
    def s_draw_object_ole(self, tag, attrs):
        ...
        if (
            class_id
            and class_id.lower() == "00020803-0000-0000-c000-000000000046"
    ...
    def s_draw_page(self, tag, attrs):
        ...
        if self.generate_css:
            self.opentag(
                "fieldset", {"class": "DP-%s MP-%s" % (stylename, masterpage)}
            )
        else:
...
"""
    ...
    def generate_stylesheet(self):
        for name in self.stylestack:
            ...
            if (
                "__style-family" in styles
                and styles["__style-family"] in self.styledict
            ):
            ...
            while (
                "__parent-style-name" in styles
                and styles["__parent-style-name"] in self.styledict
            ):
                parentstyle = self.styledict[
                    styles["__parent-style-name"]
                ].copy()
    ...
    def s_text_list(self, tag, attrs):
        ...
        if self.generate_css:
            self.opentag(
                "%s" % self.listtypes.get(list_class, "ul"),
                {"class": list_class},
            )
        else:
