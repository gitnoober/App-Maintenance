...
class PythonInfo(object):
    ...
    def __init__(self):
        ...
        self.prefix = u(
            abs_path(getattr(sys, "prefix", None))
        )  # prefix we think
        self.base_prefix = u(
            abs_path(getattr(sys, "base_prefix", None))
        )  # venv
        self.real_prefix = u(
            ...
            ", ".join(
                ...
                for k, v in (
                    ...
                    (
                        "encoding_fs_io",
                        "{}-{}".format(
                            self.file_system_encoding, self.stdout_encoding
                        ),
                    ),
                )
                if k is not None
    ...
    def _resolve_to_system(cls, app_data, target):
        ...
        while target.system_executable is None:
            ...
            if prefix in prefixes:
                ...
                raise RuntimeError(
                    "prefixes are causing a circle {}".format(
                        "|".join(prefixes.keys())
                    )
                )
