...
_TOKEN_NOT_ALLOWED = {
    ...
    0x10: FMLA_TYPE_SHARED
    + FMLA_TYPE_COND_FMT
    + FMLA_TYPE_DATA_VAL,  # tUnion/List
    ...
    0x3B: FMLA_TYPE_SHARED
    + FMLA_TYPE_COND_FMT
    + FMLA_TYPE_DATA_VAL,  # tArea3d
...
def get_cell_range_addr(data, pos, bv, reldelta, browx=None, bcolx=None):
    if bv >= 80:
        row1val, row2val, col1val, col2val = unpack(
            "<HHHH", data[pos : pos + 8]
        )
...
def get_externsheet_local_range(bk, refx, blah=0):
    ...
    if ref_recordx == bk._supbook_addins_inx:
        if blah:
            print(
                "/// get_externsheet_local_range(refx=%d) -> addins %r"
                % (refx, info),
                file=bk.logfile,
            )
    ...
    if ref_recordx != bk._supbook_locals_inx:
        if blah:
            print(
                "/// get_externsheet_local_range(refx=%d) -> external %r"
                % (refx, info),
                file=bk.logfile,
            )
    ...
    if ref_first_sheetx == 0xFFFE == ref_last_sheetx:
        if blah:
            print(
                "/// get_externsheet_local_range(refx=%d) -> unspecified sheet %r"
                % (refx, info),
                file=bk.logfile,
            )
    ...
    if not (0 <= ref_first_sheetx <= ref_last_sheetx < nsheets):
        if blah:
            print(
                "/// get_externsheet_local_range(refx=%d) -> %r"
                % (refx, info),
                file=bk.logfile,
            )
            print(
                "--- first/last sheet not in range(%d)" % nsheets,
                file=bk.logfile,
            )
...
def get_externsheet_local_range_b57(
    ...
    if raw_extshtx > 0:
        if blah:
            print(
                "/// get_externsheet_local_range_b57(raw_extshtx=%d) -> external"
                % raw_extshtx,
                file=bk.logfile,
            )
        return (-4, -4)  # external reference
    ...
    while 0 <= pos < fmlalen:
        ...
        if not optype:
            if 0x00 <= opcode <= 0x02:  # unk_opnd, tExp, tTbl
                ...
                if blah:
                    print(
                        "   subop=%02xh subname=t%s sz=%d nc=%02xh"
                        % (subop, subname, sz, nc),
                        file=bk.logfile,
                    )
        ...
        if opcode == 0x00:  # tArray
            ...
            if not func_attrs:
                print(
                    "*** formula/tFunc unknown FuncID:%d" % funcx,
                    file=bk.logfile,
                )
                spush(unk_opnd)
            else:
            ...
            nb = 1 + int(bv >= 40)
            nargs, funcx = unpack(
                "<B" + " BH"[nb], data[pos + 1 : pos + 2 + nb]
            )
            prompt, nargs = divmod(nargs, 128)
            macro, funcx = divmod(funcx, 32768)
            if blah:
                print(
                    "   FuncID=%d nargs=%d macro=%d prompt=%d"
            ...
            if not func_attrs:
                print(
                    "*** formula/tFuncVar unknown FuncID:%d" % funcx,
                    file=bk.logfile,
                )
                spush(unk_opnd)
            else:
            ...
            if tgtobj.macro or tgtobj.binary or tgtobj.any_err:
                ...
                any_err = (
                    any_err or tgtobj.macro or tgtobj.binary or tgtobj.any_err
                )
            ...
            if bv >= 80:
                ...
                raw_extshtx, raw_shx1, raw_shx2 = unpack(
                    "<hxxxxxxxxhh", data[pos + 1 : pos + 15]
                )
                if blah:
                    print(
                        "tRef3d",
            ...
            if bv >= 80:
                ...
                refx, tgtnamex = unpack(
                    "<hxxxxxxxxH", data[pos + 1 : pos + 13]
                )
                tgtnamex -= 1
                origrefx = refx
                if refx > 0:
            ...
            if dodgy or shx1 < -1:
                ...
                if not tgtobj.evaluated:
                    ### recursive ###
                    evaluate_name_formula(
                        bk, tgtobj, tgtnamex, blah, level + 1
                    )
                if tgtobj.macro or tgtobj.binary or tgtobj.any_err:
                    if blah:
                        tgtobj.dump(
            ...
            if blah:
                print(
                    "FORMULA: /// Not handled yet: t" + oname, file=bk.logfile
                )
            any_err = 1
        if sz <= 0:
...
def decompile_formula(
    ...
    def unexpected_opcode(op_arg, oname_arg):
        msg = (
            "ERROR *** Unexpected token 0x%02x (%s) found in formula type %s"
    ...
    while 0 <= pos < fmlalen:
        ...
        if opcode == 0x00:  # tArray
            ...
            if blah:
                print(
                    "    tName: setting text to", repr(otext), file=bk.logfile
                )
            res = Operand(oUNK, None, LEAF_RANK, otext)
            spush(res)
        elif opcode == 0x04:  # tRef
            ...
            if is_rel:
                ...
                okind = oREF
            otext = cellnamerel(
                rowx, colx, row_rel, col_rel, browx, bcolx, r1c1
            )
...
def dump_formula(bk, data, fmlalen, bv, reldelta, blah=0, isname=0):
    ...
    if blah:
        print(
            "End of formula. any_rel=%d any_err=%d stack=%r"
