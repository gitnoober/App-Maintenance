...
class Resource(metaclass=DeclarativeMetaclass):
    ...
    def import_data(
        ...
        **kwargs
        ...
        using_transactions = (
            use_transactions or dry_run
        ) and supports_transactions

        if self._meta.batch_size is not None and (
            not isinstance(self._meta.batch_size, int)
            or self._meta.batch_size < 0
        ):
        ...
        **kwargs
        ...
        if using_transactions:
            if (
                ...
                or (
                    rollback_on_validation_errors
                    and result.has_validation_errors()
                )
            ):
...
class ModelResource(Resource, metaclass=ModelDeclarativeMetaclass):
    ...
    def get_fk_widget(cls, field):
        ...
        """
        return functools.partial(
            widgets.ForeignKeyWidget, model=get_related_model(field)
        )
