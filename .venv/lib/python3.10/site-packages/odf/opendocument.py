...
class OpenDocument:
    ...
    def build_caches(self, elt):
        ...
        assert isinstance(elt, element.Element) or isinstance(
            elt, odf.element.Element
        )
    ...
    def _used_auto_styles(self, segments):
        ...
        for e in self.automaticstyles.childNodes:
            if (
                isinstance(e, element.Element)
                and e.getAttrNS(STYLENS, "name") in stylenamelist
            ):
                stylelist.append(e)
    ...
    def addPicture(self, filename, mediatype=None, content=None):
        ...
        if content is None:
            ...
            filename = ""  # this value is only use by the assert further
        else:
            manifestfn = filename
    ...
    def _savePictures(self, anObject, folder):
        ...
        for subobject in anObject.childobjects:
            self._savePictures(
                subobject, "%sObject %d/" % (folder, subobjectnum)
            )
            subobjectnum += 1
    ...
    def save(self, outputfile, addsuffix=False):
        ...
        if outputfile == "-":
            ...
            if addsuffix:
                outputfile = outputfile + odmimetypes.get(
                    self.mimetype, ".xxx"
                )
    ...
    def _saveXmlObjects(self, anObject, folder):
        ...
        if self == anObject:
            self.manifest.addElement(
                manifest.FileEntry(fullpath="/", mediatype=anObject.mimetype)
            )
        else:
            self.manifest.addElement(
                manifest.FileEntry(
                    fullpath=folder, mediatype=anObject.mimetype
                )
            )
        # Write styles
        self.manifest.addElement(
    ...
    for prefix in requestedPrefixes:
        if " xmlns:{prefix}".format(prefix=prefix) not in xmlpart:
            ...
            try:
                ...
                toInsert = ' xmlns:{prefix}="urn:oasis:names:tc:opendocument:xmlns:{prefix}:1.0"'.format(
                    prefix=prefix
                )
...
def __detectmimetype(zipfd, odffile):
    ...
    for mentry, mvalue in manifest.items():
        if mentry == "/":
            assert type(mvalue["media-type"]) == type("")
...
def load(odffile):
    ...
    for mentry, mvalue in manifest.items():
        ...
        # Load subobjects into structure
            ...
            mentry[:7] == "Object " and len(mentry) < 11 and mentry[-1] == "/"
        ):
