...
from .antlr import EOF
from .antlr import CommonToken as Tok
from .antlr import TokenStream, TokenStreamException
...
name_pattern = r"\w[\.\w]*"
quotename_pattern = (
    r"'(?:[^']|'')*'"  #### It's essential that this bracket be non-grouping.
)
ne_pattern = r"<>"
ge_pattern = r">="
le_pattern = r"<="
...
class Lexer(TokenStream):
    ...
    def match_pattern(self):
        ...
        self._pos = m.end(0)
        return Tok(
            type=_toktype[m.lastindex], text=m.group(0), col=m.start(0) + 1
        )
...
if __name__ == "__main__":
    try:
        for t in Lexer(
            """ 1.23 456 "abcd" R2C2 a1 iv65536 true false if choose a_name 'qname' <> >= <= """
        ):
            print(t)
