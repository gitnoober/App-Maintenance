                ...
                if src.must == RefMust.COPY or not self.symlinks:
                    ...
                    for exe in exes:
                        fix_mach_o(
                            ...
                            current,
                            target,
                            self.interpreter.max_size,
                        )

    @classmethod
    def _executables(cls, interpreter):
        for _, targets, must, when in super(
            CPythonmacOsFramework, cls
...
class CPython2macOsFramework(CPythonmacOsFramework, CPython2PosixBase):
    ...
    def can_create(cls, interpreter):
        if not IS_MAC_ARM64 and super(
            CPython2macOsFramework, cls
        ).can_describe(interpreter):
...
class CPython2macOsArmFramework(
    ...
    @classmethod
    def can_create(cls, interpreter):
        if IS_MAC_ARM64 and super(CPythonmacOsFramework, cls).can_describe(
            interpreter
        ):
    ...
    def fix_signature(self):
        ...
        try:
            ...
            logging.fatal(
                "Could not change MacOS code signing on copied python exe at %s",
                exe,
            )
            raise
...
def fix_mach_o(exe, current, new, max_size):
    ...
    try:
        logging.debug(
            "change Mach-O for %s from %s to %s",
            ensure_text(exe),
            current,
            ensure_text(new),
        )
    ...
    class FileView(object):
        ...
        def _checkwindow(self, seek_to, op):
            if not (self._start <= seek_to <= self._end):
                msg = (
                    "{} to offset {:d} is outside window [{:d}, {:d}]".format(
        ...
        def seek(self, offset, whence=0):
            ...
            if whence == os.SEEK_SET:
                ...
                raise IOError(
                    "Invalid whence argument to seek: {!r}".format(whence)
                )
