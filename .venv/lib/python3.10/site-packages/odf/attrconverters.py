...
def cnv_boolean(attribute, arg, element):
    ...
    if str(arg).lower() in ("1", "true", "yes"):
        return "true"
    raise ValueError(
        "'%s' not allowed as Boolean value for %s" % (str(arg), attribute[1])
    )
...
def cnv_list_linkage_type(attribute, arg, element):
    if arg not in ("selection", "selection-indices"):
        raise ValueError(
            "'%s' is not either 'selection' or 'selection-indices'" % arg
        )
    return str(arg)
...
def cnv_NCName(attribute, arg, element):
    ...
    """
    if (sys.version_info[0] == 3 and isinstance(arg, str)) or (
        sys.version_info[0] == 2 and type(arg) in types.StringTypes
    ):
...
def cnv_points(attribute, arg, element):
    ...
    if (sys.version_info[0] == 3 and isinstance(arg, str)) or (
        ...
        try:
            ...
            raise ValueError(
                "Points must be string or [(0,0),(1,1)] - not %s" % arg
            )
        return strarg
...
attrconverters = {
    ...
    ((CHARTNS, "axis-label-position"), None): cnv_string,  # Multi-value
    ((CHARTNS, "axis-position"), None): cnv_string,  # Multi-value
    ...
    ((CHARTNS, "label-position"), None): cnv_string,  # Multi-value
    ((CHARTNS, "label-position-negative"), None): cnv_string,  # Multi-value
    ...
    ((CHARTNS, "tick-mark-position"), None): cnv_string,  # Multi-value
    ...
    ((CHARTNS, "treat-empty-cells"), None): cnv_string,  # Multi-value
