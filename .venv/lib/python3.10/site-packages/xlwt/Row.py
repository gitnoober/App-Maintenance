...
class Row(object):
    ...
    def __init__(self, rowx, parent_sheet):
        if not (isinstance(rowx, int_types) and 0 <= rowx <= 65535):
            raise ValueError(
                "row index was %r, not allowed by .xls format" % rowx
            )
        self.__idx = rowx
    ...
    def __adjust_bound_col_idx(self, *args):
        for arg in args:
            ...
            if not ((0 <= iarg <= 255) and arg == iarg):
                raise ValueError(
                    "column index (%r) not an int in range(256)" % arg
                )
            sheet = self.__parent
    ...
    def insert_cell(self, col_index, cell_obj):
        if col_index in self.__cells:
            if not self.__parent._cell_overwrite_ok:
                msg = (
                    "Attempt to overwrite cell: sheetname=%r rowx=%d colx=%d"
    ...
    def get_cells_biff_data(self):
        cell_items = [
            item for item in iteritems(self.__cells) if item[1] is not None
        ]
    ...
    def set_cell_text(self, colx, value, style=Style.default_style):
        ...
        self.insert_cell(
            colx,
            StrCell(
                self.__idx, colx, xf_index, self.__parent_wb.add_str(value)
            ),
        )
    ...
    def set_cell_mulblanks(
        self, first_colx, last_colx, style=Style.default_style
    ):
        ...
        self.insert_mulcells(
            first_colx,
            last_colx,
            MulBlankCell(self.__idx, first_colx, last_colx, xf_index),
        )
    ...
    def set_cell_date(self, colx, datetime_obj, style=Style.default_style):
        ...
        self.insert_cell(
            colx,
            NumberCell(
                self.__idx, colx, xf_index, self.__excel_date_dt(datetime_obj)
            ),
        )

    def set_cell_formula(
        self, colx, formula, style=Style.default_style, calc_flags=0
    ):
        self.__adjust_height(style)
        self.__adjust_bound_col_idx(colx)
        xf_index = self.__parent_wb.add_style(style)
        ...
        self.insert_cell(
            colx,
            FormulaCell(self.__idx, colx, xf_index, formula, calc_flags=0),
        )
    ...
    def set_cell_boolean(self, colx, value, style=Style.default_style):
        ...
        xf_index = self.__parent_wb.add_style(style)
        self.insert_cell(
            colx, BooleanCell(self.__idx, colx, xf_index, bool(value))
        )

    def set_cell_error(
        self, colx, error_string_or_code, style=Style.default_style
    ):
        self.__adjust_height(style)
        self.__adjust_bound_col_idx(colx)
        xf_index = self.__parent_wb.add_style(style)
        self.insert_cell(
            colx, ErrorCell(self.__idx, colx, xf_index, error_string_or_code)
        )
    ...
    def write(self, col, label, style=Style.default_style):
        ...
        if isinstance(label, basestring):
            ...
            label, bool
        ):  # bool is subclass of int; test bool first
            self.insert_cell(
                col, BooleanCell(self.__idx, col, style_index, label)
            )
        elif isinstance(label, int_types + (float, Decimal)):
            self.insert_cell(
                col, NumberCell(self.__idx, col, style_index, label)
            )
            ...
            self.insert_cell(
                col, FormulaCell(self.__idx, col, style_index, label)
            )
    ...
    def set_cell_rich_text(
        self, col, rich_text_list, style=Style.default_style
    ):
        self.__adjust_height(style)
        self.__adjust_bound_col_idx(col)
    ...
    def __rich_text_helper(self, col, rich_text_list, style, style_index=None):
        ...
        for data in rich_text_list:
            if isinstance(data, basestring):
                ...
                if not isinstance(data[0], basestring) or not isinstance(
                    data[1], Font
                ):
                    raise Exception(
                        "Unexpected data type %r, %r"
        ...
        if rt:
            self.insert_cell(
                col,
                StrCell(
                    self.__idx, col, style_index, self.__parent_wb.add_rt(rt)
                ),
            )
        else:
            self.insert_cell(col, BlankCell(self.__idx, col, style_index))
