    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.tag in self.parent.twotags:
            ...
            if len(args) == 0:
                for myarg, mydict in _argsdicts(args, kwargs):
                    self.render(
                        self.tag, True, myarg, mydict
                    )  # here myarg is always None, because len( args ) = 0
            else:
                raise ClosingError(self.tag)
        elif (
            self.parent.mode == "strict_html"
            and self.tag in self.parent.deptags
        ):
...
class page:
    ...
    def __str__(self):
        ...
        return self.separator.join(
            self.header + self.content + self.footer + end
        )
...
class ClosingError(MarkupError):
    def __init__(self, tag):
        self.message = (
            "The element '%s' does not accept non-keyword arguments (has no closing tag)."
            % tag
        )
...
class ArgumentError(MarkupError):
    def __init__(self, tag):
        self.message = (
            "The element '%s' was called with more than one non-keyword argument."
            % tag
        )
...
class DeprecationError(MarkupError):
    def __init__(self, tag):
        self.message = (
            "The element '%s' is deprecated, instantiate markup.page with mode='loose_html' to allow it."
            % tag
        )
...
class ModeError(MarkupError):
    def __init__(self, mode):
        self.message = (
            "Mode '%s' is invalid, possible values: strict_html, html (alias for strict_html), loose_html, xml."
            % mode
        )
