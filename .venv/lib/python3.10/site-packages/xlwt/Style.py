...
class StyleCollection(object):
    ...
    def __init__(self, style_compression=0):
        ...
        self._num_formats = {}
        for fmtidx, fmtstr in zip(
            range(0, 23), StyleCollection._std_num_fmt_list[0:23]
        ):
            self._num_formats[fmtstr] = fmtidx
        for fmtidx, fmtstr in zip(
            range(37, 50), StyleCollection._std_num_fmt_list[23:]
        ):
            self._num_formats[fmtstr] = fmtidx
    ...
    def _add_style(self, style):
        ...
        if font in self._font_id2x:
            ...
            if font_idx is not None:
                ...
                font_idx = (
                    len(self._font_x2id) + 1
                )  # Why plus 1? Font 4 is missing
                self._font_id2x[font] = font_idx
        ...
        if xf in self._xf_id2x:
            ...
            xf_key = (font_idx, num_format_idx) + tuple(
                obj._search_key() for obj in gof
            )
...
def rotation_func(s):
    try:
        ...
        return None
    if not (-90 <= value <= 90):
        raise EasyXFCallerError(
            "rotation %d: should be -90 to +90 degrees" % value
        )
    if value < 0:
...
xf_dict = {
    ...
    "font": {
        ...
        "height": IntULim(
            0xFFFF
        ),  # practical limits are much narrower e.g. 160 to 1440 (8pt to 72pt)
        "italic": bool_map,
        "name": any_str_func,
        "outline": bool_map,
        "shadow": bool_map,
...
def _parse_strg_to_obj(
    ...
    for line in _esplit(strg, line_sep, esc_char):
        ...
        if len(split_line) != 2:
            raise EasyXFCallerError(
                'line %r should have exactly 1 "%c"' % (line, intro_sep)
            )
        ...
        for counter in range(2):
            ...
            if not isinstance(result, str):
                raise EasyXFAuthorError(
                    "section %r should map to dict or str object; found %r"
                    % (section, type(result))
                )
            # synonym
            old_section = section
            ...
            raise EasyXFAuthorError(
                "Attempt to define synonym of synonym (%r: %r)"
                % (old_section, result)
            )
        section_dict = result
        ...
        if section_obj is None:
            raise EasyXFAuthorError(
                "instance of %s class has no attribute named %s"
                % (obj.__class__.__name__, section)
            )
        for kv_str in _esplit(item_str, field_sep, esc_char):
            ...
            if not v:
                raise EasyXFCallerError(
                    "no value supplied for %s.%s" % (section, k)
                )
            for counter in xrange(2):
                result = section_dict.get(k)
                if result is None:
                    raise EasyXFCallerError(
                        "%s.%s is not a known attribute" % (section, k)
                    )
            ...
            for value_rule in value_info:
                if isinstance(value_rule, dict):
                    ...
                    raise EasyXFAuthorError(
                        "unknown value rule for attribute %r: %r"
                        % (k, value_rule)
                    )
            else:
                raise EasyXFCallerError(
                    "unexpected value %r for %s.%s" % (v, section, k)
                )
            try:
                orig = getattr(section_obj, k)
            except AttributeError:
                raise EasyXFAuthorError(
                    "%s.%s in dictionary but not in supplied object"
                    % (section, k)
                )
            if debug:
                print(
                    "+++ %s.%s = %r # %s; was %r"
                    % (section, k, value, v, orig)
                )
