...
class ODF2MoinMoin(object):
    ...
    def extractTextProperties(self, style, parent=None):
        ...
        textProps.setUnderlined(
            textPropEl.getAttribute("style:text-underline-style")
        )
        textProps.setStrikethrough(
            textPropEl.getAttribute("style:text-line-through-style")
        )
    ...
    def processStyles(self, styleElements):
        ...
        for style in styleElements:
            ...
            if family == "text":
                ...
                self.paragraphStyles[name] = self.extractParagraphProperties(
                    style, parent
                )
                self.textStyles[name] = self.extractTextProperties(
                    style, parent
    ...
    def load(self, filepath):
        ...
        fontfacedecls = styles_doc.getElementsByTagName(
            "office:font-face-decls"
        )
        if fontfacedecls:
            self.processFontDeclarations(fontfacedecls[0])
        self.processStyles(styles_doc.getElementsByTagName("style:style"))
        self.processListStyles(
            styles_doc.getElementsByTagName("text:list-style")
        )

        self.content = xml.dom.minidom.parseString(zip.read("content.xml"))
        fontfacedecls = self.content.getElementsByTagName(
            "office:font-face-decls"
        )
        if fontfacedecls:
            self.processFontDeclarations(fontfacedecls[0])
        ...
        self.processListStyles(
            self.content.getElementsByTagName("text:list-style")
        )
    ...
    def draw_image(self, node):
        ...
        if (
            link and link[:2] == "./"
        ):  # Indicates a sub-object, which isn't supported
            return "%s\n" % link
    ...
    def text_note(self, node):
        cite = (
            node.getElementsByTagName("text:note-citation")[0]
            .childNodes[0]
            .nodeValue
        )
